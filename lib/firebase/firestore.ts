...
export interface TemporaryStory {
  ...
  views?: number
  viewedBy?: string[]
  isTemporary?: boolean // Explicitly mark as temporary story
}
...
// Now using only getCreatorActiveStories with the temporaryStories collection
export const getCreatorActiveStories = async (creatorId: string): Promise<TemporaryStory[]> => {
  try {
    ...
    const now = new Date()
    const q = query(storiesRef, where("creatorId", "==", creatorId))
    ...
    const stories = querySnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
      isTemporary: true, // Explicitly mark as temporary story
    })) as TemporaryStory[]
...
export const deleteExpiredStories = async (): Promise<void> => {
  try {
    const storiesRef = collection(db, "temporaryStories")
    const now = new Date()
