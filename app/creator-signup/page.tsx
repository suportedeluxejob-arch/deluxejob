...
export default function CreatorSignupPage() {
  ...
  useEffect(() => {
    ...
    if (referralCodeFromUrl) {
      console.log("[v0] Referral code from URL:", referralCodeFromUrl)
      ...
      const autoValidate = async () => {
        ...
        try {
          const codeData = await validateReferralCode(referralCodeFromUrl.toUpperCase())

          if (codeData) {
            console.log("[v0] Auto-validated referral code successfully")
            setCodeValidated(true)
            setErrors((prev) => ({ ...prev, referralCode: "" }))
          } else {
            console.log("[v0] Auto-validation failed - invalid code")
          ...
          console.error("[v0] Error auto-validating referral code:", error)
          setCodeValidated(false)
          setErrors((prev) => ({
            ...prev,
