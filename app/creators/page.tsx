...
import { Search, Verified, Eye, Crown, TrendingUp, Loader2 } from "lucide-react"
import { useRouter } from "next/navigation"
import { getCreatorsPaginated, type CreatorProfile, getCreatorActiveStories } from "@/lib/firebase/firestore"
...
export default function CreatorsPage() {
  ...
  const [selectedCreatorForStory, setSelectedCreatorForStory] = useState<CreatorProfile | null>(null)
  ...
  const loadCreators = useCallback(
    async (isInitial = false) => {
      try {
        ...
        } = await getCreatorsPaginated(isInitial ? undefined : lastVisible, 20)
        ...
        const creatorsToCheckStories = isInitial ? newCreators.slice(0, 10) : newCreators.slice(0, 5)
        const batchSize = 3

        for (let i = 0; i < creatorsToCheckStories.length; i += batchSize) {
          const batch = creatorsToCheckStories.slice(i, i + batchSize)

          const storiesResults = await Promise.all(
            batch.map(async (creator) => {
              try {
                const stories = await getCreatorActiveStories(creator.uid)
                return { uid: creator.uid, hasStories: stories.length > 0 }
              } catch (error) {
                return { uid: creator.uid, hasStories: false }
              }
            }),
          )

          setCreatorsWithStories((prev) => {
            const newSet = new Set(prev)
            storiesResults.forEach((result) => {
              if (result.hasStories) {
                newSet.add(result.uid)
              }
            })
            return newSet
          })
  ...
  }, [hasMore, isLoadingMore, isLoading, loadCreators])
  ...
  useEffect(() => {
    ...
    if (selectedFilter === "gratuitos") {
      ...
      filtered = filtered
        .sort((a, b) => {
          const aScore = (a.followerCount || 0) + (a.contentCount || 0) * 2
          const bScore = (b.followerCount || 0) + (b.contentCount || 0) * 2
            ...
            <>
                    ...
                    <div className="relative h-20 sm:h-24">
                      ...
                      <div className="absolute bottom-2 left-2 sm:left-3">
                        <StoryRingAvatar
                          ...
                          hasActiveStory={creatorsWithStories.has(creator.uid)}
                          size="lg"
                          onClick={() => {
                            if (creatorsWithStories.has(creator.uid)) {
                              setSelectedCreatorForStory(creator)
                            }
                          }}
                      ...
                      {creator.pricing && (creator.pricing.premium || creator.pricing.diamante) && (
                ...
                <div ref={observerTarget} className="py-8 flex justify-center">
                  {isLoadingMore && (
                    <div className="flex items-center space-x-2 text-muted-foreground">
                      ...
                      <span className="text-sm">Carregando mais criadoras...</span>
                    </div>
                  )}
                </div>
              )}
              ...
              {!hasMore && filteredCreators.length > 0 && (
                <div className="py-8 text-center text-muted-foreground text-sm">
                  Você viu todas as criadoras disponíveis
                </div>
              )}
            </>
          )}
